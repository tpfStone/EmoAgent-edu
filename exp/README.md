...
在线交互链路应保持轻量：首次对话走 `F1 -> F2 -> F3 -> 流式返回`，F4 critic 放到后台异步执行。后续轮次优先走轻量 CBT 支持，并在后台 F4 指导已经完成时把建议注入生成 prompt。这样既保留多智能体研究链路的理论支撑，也避免把产品交互拖到不可用。
...
F3 已经验证 PsyQA 标注数据可以作为策略先验和 support card。`c1 情感共情型` 和 `c2 认知共情型` 的方向在小样本和扩展样本中都能被区分，但完整双候选加 critic 的在线耗时过高，因此生产侧采用“先判断用户支持模式，再生成单候选”的方式。

F4 critic 当前更适合后台质量评估、session guidance 和后续 DPO 数据构造，不适合在线强阻塞。模型 judge 在合成控制样本上表现较稳，但和人工偏好的对齐仍不足，需要更多人工校准后再作为偏好训练依据。

## 目录结构

| 路径 | 内容 | 保留原因 |
| --- | --- | --- |
| `data/psyqa_labelled.json` | 已去重整合的 PsyQA 标注数据 | 后续 F1/F3/F4 的共同数据基础 |
...
原始 PsyQA 数据先经过 `psyqa_labelled.py` 调用 DashScope/DeepSeek 兼容接口做结构化标注，输出字段包括：
...
| 方向 | 目标 |
| --- | --- |
| `c1 情感共情型` | 更强 ER，适合强情绪、需要被接住的首轮对话 |
| `c2 认知共情型` | 更强 IP，适合明确求助、需要低压可执行起点的情况 |
