...
| D8 | P1 | 已修 | `docs/figures/figure-1-runtime-boundary.svg` | 图中展示 setup 填充 RAG memory，优化阶段含人工偏好标注。 | RAG 当前为空 / 未来项；偏好对尚未通过验证，不能用于 DPO。 | 该图更像概念图，作为当前架构图会误导。 | 已改为标注 `planned RAG` 和 `F9/pairwise gate`，提醒 DPO 前必须通过验证。 |
