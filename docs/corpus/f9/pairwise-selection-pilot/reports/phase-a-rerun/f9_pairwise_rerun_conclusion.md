...
严格按 go/no-go 口径：`comparison_intersection_pairs=7`，低于 Phase A rerun 计划里的 `12` 下限，所以本轮正式结论是 `inconclusive`。它不能证明 pairwise 优于 pointwise，也不能推进到 Phase B/runtime 切换。
...
- Pairwise stable 输出明显偏斜：本轮所有 stable pairwise winner 都是 `c1`，而 human-valid 偏好接近平衡（`c1=7`、`c2=8`）。由于本包里 `c1` 始终是共情型候选，这更像是对共情型候选的强偏好，或者输入包设计无法区分取向偏好和候选身份。
...
3. 先修 F3 候选生成和 pair package 设计，再 rerun。
...
- 后续 frozen package 中随机或均衡 `c1/c2` 对应的取向，避免分析时无法区分候选身份和取向风格。
