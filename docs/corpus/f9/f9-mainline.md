...
当前主线进入 R9：先做模型因素调研与 F4-only 对照实验。固定 R8 priority 10 条候选，不重跑 F3、不改 prompt、不改 `32/40` gate，用当前 `deepseek-chat` 作为 baseline，对比 `deepseek-v4-pro` 的 F4 复评是否更接近人工。R9 初步执行显示：`deepseek-v4-pro` 不能直接替换为 judge，完整 3 repeats 超时，fallback pilot 10/10 行触发 `llm_parse_failure`，因此不能把 0 分当成“更接近人工”。
...
| R9 | priority 10 条人工复核显示候选普遍质量差，不能只讨论 gate 阈值 | 固定这 10 条候选，做 F4-only 模型对照：`deepseek-chat` baseline vs `deepseek-v4-pro` | baseline 完成；v4-pro 完整 3 repeats 超时，fallback pilot 10/10 `llm_parse_failure` | v4-pro 不能 drop-in 升级；下一步若继续模型路线，先做结构化输出兼容性/原始响应诊断 |
...
### 阻塞点 1：priority 10 条显示质量普遍偏差，baseline F4 仍偏宽

R8 priority 10 条人工判断中，ER 仅 1/10 认可应得 2，IP 0/10 认可应得 2。`deepseek-chat` baseline 在 20 个 ER/IP 标签里只匹配 10 个，说明现有 F4 judge 对高分侧仍偏宽。

### 阻塞点 2：v4-pro 不能直接作为 F4 judge 替换

`deepseek-v4-pro` 完整 3 repeats 超时；fallback pilot 虽完成，但 10/10 行均触发 `llm_parse_failure`。因此本轮没有有效 candidate accuracy 数据，不能判断它是否更接近人工，也不能直接升级。

当前存在三种可能：

| 可能性 | 含义 | 下一步 |
|---|---|---|
| A：结构化输出兼容性问题 | v4-pro 能理解任务，但没有稳定按 F4 JSON 输出 | 先做 raw response / JSON mode 诊断，再决定是否继续 |
| B：模型成本过高 | v4-pro 可用但太慢，不适合每条候选 3x median judge | 只考虑少量复核，不作为主 judge |
| C：模型能力仍不够 | 修复输出格式后仍不贴近人工 | 转向 F3 生成策略和人工标注锚点 |
...
下一步不是继续泛泛调 prompt，也不是让人工继续看完 30 条 backup，而是先处理 R9 暴露的模型兼容性问题：

1. 保留当前 R9 产物：`docs/corpus/f9/validation-stability/model-eval/`。
2. 不采纳 v4-pro 当前 0/0 结果，因为它们是 parse failure。
3. 若继续模型路线，新增一个最小 raw-response 诊断：只跑 1-2 条 priority，捕获 v4-pro 原始输出，判断是 JSON 格式问题、模型名/接口问题，还是 F4 prompt 不兼容。
4. 若不继续模型路线，则回到 F3 生成策略：减少“复述 + 加深情绪 + 分析提问”的默认生成模式。
5. 在这两个分支未决前，不启动正式人工 F9。
