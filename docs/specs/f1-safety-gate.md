...
**当前状态**：已接入运行时。`/chat` 前置安全门默认使用 `ClassifierSafetyGateService` 和本地 `F1SafetyClassifier`；生产接口为 `/api/safety/classifier/evaluate`。旧的 LLM prompt 版 `/api/safety/evaluate` 仍保留，用于兼容和对照实验。`OrchestratorService` 在 yellow/red 时短路下游 F2/F3，返回固定转介话术。
...
接收用户当前消息 + 最近 N 轮对话历史，判定风险等级（green/yellow/red），输出等级与转介动作。**绿色放行进入 F2；黄/红中断生成，返回转介内容。** 本模块只做风险分级与转介，不生成辅导对话。
...
> **重要**：公开输出仍是三分类标签。分类器内部会计算三类概率，但后续管线只依据最终 `risk_level` 分支：green→进入 F2；yellow/red→直接把 `referral_message` 返回用户，**不进生成**。
