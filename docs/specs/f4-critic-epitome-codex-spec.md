...
CASEL 辅助维（仅当 `activated_casel` 非空时评，0/1/2 同上量纲，定义见方案 md §六）：作教育价值兜底，权重低于 EPITOME。MVP 实现中，CASEL 评分并入现有 F4 judge prompt，不新增第二次 LLM 调用。
...
- **加权总分** = w_ER·ER + w_IP·IP + w_EX·EX + Σ(w_casel·casel_i)
- **初始权重（等权起步，不依赖专家）**：EPITOME 三维各权重 = 1.0；CASEL 辅助维各 = 0.5。🚧 DPO 数据积累后反推优化。
...
- [ ] 等权计分，权重做成可配置常量
- [ ] `activated_casel` 非空时 CASEL 分数进入 `weighted_total`；为空时保持 EPITOME-only 行为
