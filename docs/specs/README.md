...
本目录是实现契约层，负责说明当前代码行为、已经落地的 runtime 边界，以及还未进入运行时的后续验证 gate。根 README 只保留运行入口；实验细节在 `../../exp/README.md`。

## 当前运行时

- `/chat` / `/chat/stream` 首轮：F1 本地安全门 -> F2 情境/支持模式/二次安全 -> F3 单候选流式返回 -> 后台 F4 guidance。
- 后续对话：仍先过 F1 本地安全门，然后轻量 CBT-compatible 生成并注入最近历史；只有后台 F4 guidance 已完成时才注入，不等待。
...
- `GET /api/critic/guidance/{session_id}`：研究/诊断接口，只读查看后台 F4 guidance 的 `missing`、`pending`、`ready`、`failed` 状态；`ready` 时包含后台 F4 `scores` 供研究台展示，不改变 `/chat` 行为。
...
后续轮次仍先做 F1 安全门；不再每次调用完整 F2/F4，目的是降低延迟并让对话自然延续。F4 guidance 如果还没生成完成，不等待、不阻塞。
